...
AnalysisResult performGLCMAnalysis(const cv::Mat &inputImage) {
  ...
  if (glcmInputMat.empty() || glcmInputMat.type() != CV_8UC1) {
    result.detailedLog += QCoreApplication::translate(
        "Analysis", "\nError: Failed to prepare 8-bit single channel image "
                    "required for GLCM.");
    result.overviewSummary =
        overviewPrefix +
  ...
  result.detailedLog +=
      QCoreApplication::translate(
          "Analysis", "\nCalculating GLCM with offset (dx=%1, dy=%2), "
                      "levels=%3, symmetric=true, normalized=true\n")
          .arg(dx)
          .arg(dy)
          .arg(levels);
  ...
  try {
    ...
    result.detailedLog +=
        QCoreApplication::translate(
            ...
            .arg(dy);
    result.detailedLog +=
        QCoreApplication::translate("Analysis", "Contrast: %1\n").arg(contrast);
    result.detailedLog +=
        QCoreApplication::translate("Analysis", "Correlation: %1\n")
            .arg(correlation);
    result.detailedLog +=
        QCoreApplication::translate("Analysis", "Energy (ASM): %1\n")
            .arg(energy);
    result.detailedLog +=
        QCoreApplication::translate("Analysis", "Homogeneity (IDM): %1\n")
            .arg(homogeneity);
    ...
    result.detailedLog +=
        QCoreApplication::translate(
            "Analysis", "Internal Log: GLCM features calculated: Contrast=%1, "
                        "Correlation=%2, Energy=%3, Homogeneity=%4\n")
            .arg(contrast)
            .arg(correlation)
            .arg(energy)
            .arg(homogeneity);
    ...
    result.detailedLog +=
        QCoreApplication::translate(
            "Analysis", "Internal Log: OpenCV Error during GLCM analysis: %1\n")
            .arg(QString::fromStdString(e.what()));
    result.success = false;
  }
...
cv::Mat prepareImageForGLCM(const cv::Mat &inputImage, QString &log) {
  ...
  if (inputImage.channels() == 2) {
    log += QCoreApplication::translate(
        "AnalysisHelper",
        "Input is complex (2-channel), calculating magnitude.\n");
    ...
    if (inputImage.depth() != CV_8U && inputImage.depth() != CV_16U &&
      ...
      log += QCoreApplication::translate(
                 "AnalysisHelper", "Input depth (%1) not directly supported by "
                                   "cvtColor, converting to CV_8U first.\n")
    ...
    if (sourceForCvt.channels() >= 3) {
      try {
        ...
        log += QCoreApplication::translate(
                   "AnalysisHelper",
                   "Standard grayscale conversion failed (%1), likely not BGR "
                   "format. Falling back to first channel.\n")
                   .arg(QString::fromStdString(e.what()));
        std::vector<cv::Mat> channels;
      ...
      log += QCoreApplication::translate(
                 "AnalysisHelper", "Input has %1 channels, not standard "
                                   "BGR/BGRA. Using first channel directly.\n")
    ...
    log += QCoreApplication::translate("AnalysisHelper",
                                       "Input is already single-channel.\n");
    analysisMat = inputImage.clone();
  } else {
    log += QCoreApplication::translate("AnalysisHelper",
                                       "Error: Input image has 0 channels.\n");
    return cv::Mat();
  }
  ...
  if (analysisMat.empty()) {
    log += QCoreApplication::translate(
        "AnalysisHelper",
        "Error: Failed to obtain a single channel image from the input.\n");
    return cv::Mat();
  }
  ...
  if (analysisMat.depth() != CV_8U) {
    log += QCoreApplication::translate(
               "AnalysisHelper", "Normalizing single-channel image (type: %1) "
                                 "to 8-bit (0-255) for GLCM.\n")
               .arg(cv::typeToString(analysisMat.type()));
    double minVal, maxVal;
    cv::minMaxLoc(analysisMat, &minVal, &maxVal);
    if (maxVal > minVal) {
      analysisMat.convertTo(outputMat, CV_8U, 255.0 / (maxVal - minVal),
                            -minVal * 255.0 / (maxVal - minVal));
    } else {
      analysisMat.convertTo(outputMat, CV_8U);
      log += QCoreApplication::translate(
          "AnalysisHelper",
          "Image has constant value after conversion to single channel.\n");
    }
  } else {
    log += QCoreApplication::translate(
        "AnalysisHelper", "Single-channel image is already 8-bit (CV_8U).\n");
  ...
  if (outputMat.empty() || outputMat.type() != CV_8UC1) {
    log += QCoreApplication::translate(
        "AnalysisHelper",
        "Error: Failed to produce a valid CV_8UC1 image for GLCM.\n");
    return cv::Mat();
  }

  log += QCoreApplication::translate(
      "AnalysisHelper",
      "Successfully prepared CV_8UC1 image for GLCM analysis.\n");
  return outputMat;
}
...
void computeGLCM(const cv::Mat &img, cv::Mat &glcm, int dx, int dy, int levels,
  ...
  CV_Assert(img.type() == CV_8UC1 &&
            "Input image must be 8-bit single-channel (CV_8UC1)");
  ...
  if (normalize) {
    if (totalPairs > 0) {
      ...
      log += QCoreApplication::translate(
                 "AnalysisHelper",
                 "GLCM calculation complete. Normalized by %1 pairs.\n")
                 .arg(totalPairs);
    } else {
      glcm = cv::Mat::zeros(levels, levels, CV_64F);
      log += QCoreApplication::translate(
          "AnalysisHelper", "Warning: No valid pixel pairs found for GLCM "
                            "calculation. GLCM is zero.\n");
    }
  } else {
    log += QCoreApplication::translate("AnalysisHelper",
                                       "GLCM calculation complete. Contains "
                                       "raw counts (%1 total pairs).\n")
               .arg(totalPairs);
  }
}
...
void calculateGLCMFeatures(const cv::Mat &glcm, int levels, double &contrast,
  ...
  CV_Assert(glcm.rows == levels && glcm.cols == levels &&
            "GLCM dimensions must match levels");
  ...
  bool needsNormalizationAdjustment =
      std::abs(glcmSum - 1.0) > 1e-6 && glcmSum > 1e-9;
  if (needsNormalizationAdjustment) {
    log += QCoreApplication::translate(
               "AnalysisHelper", "Warning: GLCM sum (%1) is not close to 1. "
                                 "Adjusting means/stddev based on sum.\n")
               .arg(glcmSum);
    mean_i /= glcmSum;
    mean_j /= glcmSum;
    for (int i = 0; i < levels; ++i) {
    ...
    // If sum is close to 1 or 0, means are already correct relative to
    // probabilities
  }
  ...
  if (stddev_i > 1e-9 && stddev_j > 1e-9) {
    ...
    log += QCoreApplication::translate(
        "AnalysisHelper", "Warning: GLCM Correlation is undefined because "
                          "stddev_i or stddev_j is near zero.\n");
  }
  log += QCoreApplication::translate("AnalysisHelper",
                                     "GLCM feature calculation complete.\n");
}
