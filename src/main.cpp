...
int main(int argc, char *argv[]) {
  ...
  // --- i18n Setup ---
  QTranslator
      qtTranslator; // Translator for Qt standard texts (e.g., "OK", "Cancel")
  QTranslator appTranslator; // Translator for application texts

  // Detect system locale
  QLocale locale = QLocale::system();
  qDebug() << "System locale detected:"
           << locale.name(); // e.g., "zh_CN", "en_US"
  ...
  if (qtTranslator.load(locale, "qtbase", "_", qtTranslationsPath)) {
    ...
    qWarning() << "Failed to load Qt standard translations for locale"
               << locale.name();
    ...
    if (qtTranslator.load(locale, "qt", "_", qtTranslationsPath)) {
      ...
      qWarning() << "Failed to load generic Qt translations for locale"
                 << locale.name();
    }
  }
  ...
  QString appTranslationFile =
      QString(":/i18n/SAR-QualityQT_%1.qm").arg(locale.name());
  ...
  if (appTranslator.load(appTranslationFile)) {
    ...
    qWarning() << "Failed to load application translation file:"
               << appTranslationFile;
    // Fallback to generic language if specific locale (e.g., en_US) fails
    QString genericLang =
        locale.name().split('_').first(); // e.g., "en" from "en_US"
    appTranslationFile = QString(":/i18n/SAR-QualityQT_%1.qm").arg(genericLang);
    qDebug() << "Looking for generic application translation file:"
             << appTranslationFile;
    if (appTranslator.load(appTranslationFile)) {
      qDebug() << "Loaded generic application translation:"
               << appTranslationFile;
      a.installTranslator(&appTranslator);
    } else {
      qWarning() << "Failed to load generic application translation file:"
                 << appTranslationFile;
    }
  }
  // --- End i18n Setup ---
